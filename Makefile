...
# Link test_gpu_resolve (GPU hidden-resolution exactness; self-skips without CUDA)
$(TEST_GPU_RESOLVE): $(OBJECTS) $(BUILD_DIR)/$(TEST_DIR)/test_gpu_resolve.o | $(BIN_DIR)
	$(CC) $(CFLAGS) $^ $(LDFLAGS) -lpthread -o $@
	@echo "✓ Built: $@"
...
# Run tests
test: $(TEST_GAP_DETECTION) $(TEST_PRIMALITY) $(TEST_WORKER_THREADS) $(TEST_GAPCOIN_RPC) $(TEST_BLOCK_SUBMISSION) $(TEST_SIEVE_CORE) $(TEST_GPU_FERMAT) $(TEST_GPU_SIEVE) $(TEST_GPU_RESOLVE) $(TEST_GAP_DIST) $(TEST_HALFCLASS) $(TEST_GAP_PRIORITY) $(TEST_CRT_SET) $(TEST_GAP_TARGET) $(TEST_COVERING) $(TEST_CRT_RUNTIME) $(TEST_CRT_SUBMISSION)
