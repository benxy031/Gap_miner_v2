...
CC = gcc
# -MMD -MP: emit a .d dependency file per object and track HEADERS.  Without
# this a header-only change (e.g. GPU_ADAPTER_MAX_BATCH 160000 -> 320000 in
# gpu_adapter.h on 2026-09-15) does NOT recompile its users, and stale objects
# silently keep the OLD constant while freshly built callers use the new one.
# That exact mismatch cost the hunt every candidate beyond index 160000 of a
# submit (silent clamp, zero verdicts) until it was found on 2026-09-17.
...
NVCC_FLAGS = -DGPU_NLIMBS=$(GPU_NLIMBS) -std=c++17 -MMD -MP
...
# Header dependencies generated by -MMD -MP (see CFLAGS).  `find` at parse time
# keeps this self-maintaining for every object (sources, gpu, tests).
...
TEST_POW_Q48 = $(BIN_DIR)/test_pow_q48
...
# Default target
all: $(BIN_DIR) $(BUILD_DIR) $(OBJECTS) $(MAIN_BINARY) $(TEST_GAP_DETECTION) $(TEST_PRIMALITY) $(TEST_WORKER_THREADS) $(TEST_GAPCOIN_RPC) $(TEST_BLOCK_SUBMISSION) $(TEST_SIEVE_CORE) $(TEST_GPU_FERMAT) $(TEST_GPU_SIEVE) $(TEST_GPU_RESOLVE) $(TEST_GAP_DIST) $(TEST_HALFCLASS) $(TEST_GAP_PRIORITY) $(TEST_CRT_SET) $(TEST_GAP_TARGET) $(TEST_POW_Q48) $(TEST_COVERING) $(TEST_CRT_RUNTIME) $(TEST_CRT_SUBMISSION) $(TEST_GAP_HUNT) $(TEST_STRATUM) $(CRT_GEN) $(GEN_CRT) $(COVER_MAX)
...
$(TEST_POW_Q48): $(BUILD_DIR)/$(SRC_DIR)/pow_q48.o $(BUILD_DIR)/$(TEST_DIR)/test_pow_q48.o | $(BIN_DIR)
	$(CC) $(CFLAGS) $^ $(LDFLAGS) -lpthread -o $@
	@echo "✓ Built: $@"
...
# Gapcoin legacy stratum client (mock pool server, no network needed)
$(TEST_STRATUM): $(BUILD_DIR)/$(SRC_DIR)/stratum.o $(BUILD_DIR)/$(TEST_DIR)/test_stratum.o | $(BIN_DIR)
	$(CC) $(CFLAGS) $^ $(LDFLAGS) -lpthread -o $@
	@echo "✓ Built: $@"
...
# Run tests
test: $(TEST_GAP_DETECTION) $(TEST_PRIMALITY) $(TEST_WORKER_THREADS) $(TEST_GAPCOIN_RPC) $(TEST_BLOCK_SUBMISSION) $(TEST_SIEVE_CORE) $(TEST_GPU_FERMAT) $(TEST_GPU_SIEVE) $(TEST_GPU_RESOLVE) $(TEST_GAP_DIST) $(TEST_HALFCLASS) $(TEST_GAP_PRIORITY) $(TEST_CRT_SET) $(TEST_GAP_TARGET) $(TEST_POW_Q48) $(TEST_COVERING) $(TEST_CRT_RUNTIME) $(TEST_CRT_SUBMISSION) $(TEST_STRATUM)
