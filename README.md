...
`GPU_SIEVE=1`) has to be asked for. On the fused CRT chain path use **four worker
threads per GPU** (`--threads` = 4 x the number of GPUs; measured optimum, the table
is under `--threads` below) — the device is assigned round-robin (`worker i -> device
i % gpu_count`) and each worker owns its own GPU contexts and gather staging, so N
workers per card run N chains with N independent mark/extract streams. The reason
more workers help is that the **host side of a chain round is serialized per worker**
(mark, extract, round trip), not the batch size: at shift507 `p74_lex_m30`, 90 s ABBA
arms, `tests/window` identical (79.6-79.8) in every arm, win/s = **13,254 (2/card)**,
14,511 (3/card), **15,376 (4/card) = +16%** (the same comparison across all four arms of both runs: 13,173 -> 15,150 = **+15.0%**), and **+21.9%** against 2 workers at the
SAME K=1024 (12,613 -> 15,376); 8 workers on ONE card drops to 11,612 (-12%; its K
clamped to 512/256 per worker). Scope:
measured on the fused CRT chain path only — non-CRT keeps its own optimum
(`--threads 8` at shift 96).

```bash
# ── Mining: fused GPU pipeline + no-test chain (fastest CRT mining mode) ─────
# 4 workers on ONE card (the measured optimum); needs a CRT cover file; submits
# only with --enable-submission. Lower the count on a card with less free VRAM
# (the K auto-clamp then shrinks the flight instead of failing).
FUSED_GPU=1 ./bin/gapminer --host 127.0.0.1 --port 31397 --user benxy031 --pass xx \
  --crt-file data/crt/m23/shift507_p74_lex_m30.txt --threads 4 --enable-gpu-fermat \
  --enable-submission --coinbase-script-hex 76a914<20-byte-hash160>88ac

# Two GPUs: 8 workers = 4 per card (round-robin device assignment: workers
# 0,2,4,6 -> device 0 and 1,3,5,7 -> device 1).
FUSED_GPU=1 ./bin/gapminer --host 127.0.0.1 --port 31397 --user benxy031 --pass xx \
  --crt-file data/crt/m23/shift1017_p130_lex_m30.txt --threads 8 --enable-gpu-fermat
...
# GPU_BITS default is 2048 (32 limbs) = h256 256 bits + shift 1792, the widest
# shift CGBN runs at TPI=8 (AL=24/28/32 instantiations added 2026-09-23; AL>32
# needs TPI=16 and an odd AL falls back to the scalar kernel). A smaller
# GPU_BITS build rejects the shifts above GPU_BITS-256.
...
| `--threads <n>` | `1` | Worker threads. **Clamped to 64 with a loud message** — the per-worker telemetry block is indexed by worker id, and before 2026-09-24 a 9th worker's per-window counter increment landed on the global stop flag (`nm`: the array was `0x800` bytes at `0x14e0280` with `g_stop_requested` immediately after it at `0x14e0a80`), so `--threads 9+` silently stopped the whole run after ~1 header while the GPUs sat at 0-2%. **On the fused CRT chain path the measured optimum is 4 per GPU** (`--threads` = 4 x the number of GPUs), not one per GPU: the host side of a chain round (mark / extract / round trip) is serialized inside a worker, so a second, third and fourth worker per card add host parallelism and keep more rounds in flight at the same `tests/window`. Measured at shift507 `p74_lex_m30` (RTX 3070, i3-10100 4C/8T, 90 s ABBA arms, dry-run at `--merit 16`, `tests/window` identical 79.6-79.8 in every arm): win/s = **13,295 / 13,212 (2 workers, K=2048, the previous default packing)**; 12,688 / 12,538 (2 workers at K=1024); 14,982 / 14,039 (3 workers, K=2048); **15,053 / 15,698 (4 workers, K=1024) = +16.0%** vs 2 workers at the old packing and **+21.9%** vs 2 workers at the same K=1024; 6 and 8 workers on ONE card are WORSE (13,412-14,199 and 11,452-11,772: the box has 4 physical cores, so beyond 4 the host work oversubscribes the CPU). The gain survives the MR ceiling argument: at 2 workers the card is only 85-90% GPU-busy (`GPU MR acc/wall` 0.85-0.90), at 4 it is queue-saturated (1.41-1.43). Per-window yield is unchanged within sampling (`per Mwin` 114.8-151.6 across all six arms, consistent with Poisson on n=130-178), so blocks/h scale with win/s. **CONFIRMED on the fleet** (2x RTX 3060 12 GB, 12 cores, shift720 `p98_lex_m42`, `--sieve-primes 11000000`, live merit, 90 s arms, ABBA): win/s = 8,966 (2/card) / 9,536 & 9,453 (3/card) / **9,686 & 9,587 (4/card)** = **+6.6% over 2/card**; 5/card 9,634 & 9,888 (inside the arm spread) and 6/card 9,373 & 9,384 (**-2.7%**), so 4 per card is the optimum on a 12-core box too, not just on the 4-core dev box. Scope: measured on the fused CRT chain path (`FUSED_GPU=1` + `MINING_JUMP2`) only; the non-CRT path has no chain and keeps its own optimum |
...
| `GPU_FERMAT_TPI` | `8` | Threads per CGBN arithmetic instance (= per MR candidate) on the wide CGBN MR kernels (768/1024/1280/1536/1792/2048-bit, AL=12/16/20/24/28/32; the AL=24/28/32 widths were added 2026-09-23 with the 2048-bit default build, and AL=32 is the last width that keeps TPI=8 valid): `8`, `16` or `32`; any other value (including `4`) falls back to `8` (`4` is impossible: `dlimbs = ceil(AL/TPI)` would need the unimplemented multi-limb path at AL=12). Fewer limbs per thread means more inter-thread carry traffic per Montgomery step, so the largest TPI is the worst: **the default 8 is optimal (measured 2026-09-21)**, devbox 3070, shift998 walker (AL=20/1280-bit, `GAP_HUNT_BATCH=128`, chunk 32, `--gap-hunt-min-merit 20`, 60 s arms, last 4 ticks) -- **TPI=8: 779/788/820/834 win/s, TPI=16: 551/573/577/581 (-30%), TPI=32: 345/348/356/356 (-58%)**. A/B knob only; do not set it in production |
| `GPU_FERMAT_CGBN_SOA` | `1` | Layout of the CGBN MR candidate buffer: `1` = structure-of-arrays (`cgbn_fermat_kernel_soa_t`, host packs the limbs), `0`/`n`/`f` = array-of-structures (plain memcpy). Affects the **batch** submit path only (`gpu_fermat_submit_locked`); the chain's own walk kernel is a separate AoS kernel. **Measured 2026-09-24** (devbox 3070, `bench_fermat 12 <batch> 12`): AoS is **+11.3% at batch 2000 (1,003k vs 901k cand/s) and +7.5% at batch 20000 (1,453k vs 1,352k)** — but in the production pipeline (`FUSED_GPU=1`, `shift507_p74_lex_m30`, 3 workers, 60 s arms) the difference is **0.03% (14,259 vs 14,255 win/s, GPU acc/wall 0.915 vs 0.913)**: there the host-side SoA transpose cost and the kernel-side gain cancel. The default therefore stays SoA. Example: `GPU_FERMAT_CGBN_SOA=0 ./bin/bench_fermat 12 20000 12` |
...
| `GPU_MR_KERNEL` | `cgbn` (unset) | Measurement hook: `cios` selects the **32-bit-limb CIOS** MR kernel instead of CGBN — one thread per candidate, all limbs in registers, no shuffles and no barriers, so a warp carries 32 independent candidates in lockstep instead of CGBN TPI=8's 4. Instantiated for AL 5/6/12/16/32 (320/384/768/1024/2048-bit; 16 and 32 are the `--gap-hunt` widths at shift 720 and 1784); every other width stays on CGBN, so setting this never regresses a shift whose width has no instantiation. The wide instantiations need `cios_mr_base2`'s slim live set (n, y and the accumulator only — Mont(1) is recomputed after the squaring loop and the exponent bits are read out of n); the previous 6-array shape is what capped it at 24 limbs. **Do not "optimize" the outer multiply loop back to a full unroll: nvcc 12.4 (V12.4.131) segfaults on the 64-limb instantiation ("cicc: Segmentation fault", no diagnostic) and needs minutes on the smaller ones — `#pragma unroll 1` on that one loop keeps every width compiling in seconds.** **Any other value (unset, `cgbn`, a typo) keeps the CGBN/scalar default, so rollback is unsetting the variable.** Correctness: `tests/test_gpu_fermat` passes in both modes (0 mismatches, 0 false negatives vs GMP, AoS + SoA, AL=5/12/16/32 on CIOS, including partial-top-limb and half-width candidates). **Measured 2026-09-22 — the standalone win does NOT transfer to the chain:** in a pure large-batch MR stream (`tools/bench_cios_mr.cu`, GMP-validated, 8 streams x batch 2048) CIOS does **1,734,623 cand/s vs CGBN TPI=8's 1,387,762 (+25%)**, but end-to-end on the devbox 3070 (shift507 lex_m31, fused chain, `scripts/ab_mr_kernel.sh`) it is **slower**: 8 workers (VRAM-clamped to K=256-512) 7,260/7,007 vs CGBN 11,160/10,779 win/s = **-35%**; 4 workers (K=1024) 13,722 vs 14,924 = **-8%**; 2 workers (K=2048) 11,232 vs 12,393 = **-9%**. Reason: 1 thread per candidate needs ~16-24k threads in flight to fill 48 SMs, while the chain submits only a few hundred to ~2k candidates per MR launch; CGBN's 8-lanes-per-candidate shape fills the machine at any batch size. CIOS shows a *higher* `acc/wall` (more GPU-busy time per unit work) at *lower* win/s, i.e. it is more GPU-expensive in this regime, not less. **That is why the 2026-09-26 reopen targeted `--gap-hunt` instead** — the walk's jump2 rounds batch ~11.7k candidates per MR launch (33 windows x ~355 tests) — **and it still loses, at both large-shift widths**: devbox 3070, quiet GPU, fixed covers, fresh state file per arm, 25 s arms, 2 reps (win/s; µs per MR test in brackets): **shift 720 / 1024-bit (AL=16) CGBN 2095.5 / 2077.6 (1.25) vs CIOS 1359.8 / 1347.1 (2.18) = -35%**; **shift 1784 / 2048-bit (AL=32) CGBN 299.2 / 298.4 (8.50) vs CIOS 136.8 / 136.4 (18.95) = -54%**; tests per window are identical within the arms (268.3 vs 267.9 and 354.7 vs 353.5), so the whole delta is MR time (MR = 71/81% of the window at 720, 91/96% at 1784). Mechanism (`cuobjdump -res-usage` on the shipped binary): one thread per candidate hits the register ceiling as the width grows — 126 regs/192 B stack at 24 limbs, 172/256 at 32, **255 regs/576 B stack at 64** — while CGBN TPI=8 runs 86 regs/0 spill at 2048-bit, i.e. CIOS pays local-memory spill traffic for the arithmetic CGBN keeps in registers. Keep the instantiations (opt-in, zero production risk) for future backends: a 28-SM card needs far fewer threads in flight than this 46-SM devbox, so the shape is not universally ruled out. Example: `GPU_MR_KERNEL=cios FUSED_GPU=1 ./bin/gapminer --crt-file data/crt/m23/shift507_p74_lex_m31.txt --threads 2 --enable-gpu-fermat` |
...
| `MINING_JUMP2` | **on** | **No-test chain** (Horizon M19-style certificates) on the fused CRT mining path: per-window backward-search chain in chunked MR rounds shared across K flight windows (~3× fewer MR tests per window; stops at the merit frontier — every skipped interior gap is provably below threshold). **Default on since 2026-09-15** (measured +70–76% windows/s, see below); `MINING_JUMP2=0` restores the full scan. **Inert without `FUSED_GPU=1`** (the chain lives on the fused pipeline; without that gate `chain_on` stays 0 and the run is a plain full scan). Works with `HALF_CLASS`/`QUARTER_CLASS` (the chain walks the class-filtered survivor list; reported visible pairs are identical to the full scan's and hidden interiors are resolved by the existing on-demand machinery). Each worker owns its GPU contexts (gather staging is per-worker), so the chain runs on every worker independently; `--threads` is therefore the number of chains and the measured optimum is **4 per card** (see the packing table under `--threads`), not one per GPU. Fail-closed: any chain error falls back to a full scan of the same flight, then to the CPU path. Disables the gap-dist health histogram (the frontier cut distorts small-gap frequencies, same policy as `HALF_CLASS`). Parity-verified via `MINING_JUMP2_VERIFY` vs the full scan: **0 mismatched merit-candidate sets** (2370 flights at shift507, 1173 at shift512 rows=64, 1138 at `CRT_ROWS_BATCH=0`). A/B on the dev host (RTX 3070, fused chain, 1 thread, default chunk 32, `GPU_MARK_SPLIT` on, 90 s per point, order swapped): shift512 `p75_lex_m30` **2353 / 2300 win/s full-scan → 4002 / 3994 with the chain (+70%)**, shift507 `p74_lex_m30` **2416 / 2423 → 4275 / 4250 (+76%)**, with expensive MR tests per window **620.7 / 622.0 → 120.1 / 120.5 (5.2× fewer)** at the same settings. (An earlier 300 s A/B measured +41%/+51% — the gain is larger now because the mark kernel no longer dominates the window.) Class modes (parity-verified, 0 mismatches): `HALF_CLASS` 1973 → **2630 (+33%)**, `QUARTER_CLASS` 928 → **1018 (+9.7%)** (quarter stays host-bound at 1 thread). 2 chain workers on one 3070: `3639 win/s` vs `2953` for one. Example: `MINING_JUMP2=0 FUSED_GPU=1 ./bin/gapminer --crt-file data/crt/m23/shift507_p74_lex_m30.txt --threads 1 --enable-gpu-fermat` |
| `MINING_JUMP2_BATCH` | `2048` | Windows per chain flight (`1..4096`; the compile-time cap was raised 128 → 256 → 512 on 2026-09-18, then 512 → 1024 (2026-09-19) and 1024 → 4096 (2026-09-21, together with the candidate-buffer right-sizing that made the larger K affordable) after the 2026-09-17 "K=256 corrupts the emitted set" verdict was traced to the **verifier**, not the chain: `MINING_JUMP2_VERIFY` replayed the whole flight's full scan in ONE submit (256 x ~1800 = 455k candidates at shift512, 256 x ~2676 = 685k at shift1017) and the MR staging cap (320000) truncated it, so the verifier reported the untestable tail as `bad`; the replay is now sliced at the context cap and every size verifies clean — K=256: 214 flights at shift512 and 67 at shift1017, K=512: 123 flights, all `bad_windows=0 bad_pairs=0`, while the chain's own submits are K x chunk = 16k candidates, far below the cap). **This batch is a lever that has not saturated:** shift512 5264 (K=128) → 6113 (K=256) → **6725 win/s (K=512)**; shift1017 1498 → 1788 → **2014 win/s**. The cost is device memory — the sieve sizes its candidate buffers as `2 x odd_interval_size x K x limbs x 8 B` (shift512 K=512 ≈ 1.6 GB, shift1017 K=512 ≈ 4.6 GB; measured 5.6/8 GB used) — so lower K on cards with less free VRAM; an allocation failure fails closed to the CPU path, never to a wrong result. The per-round gather/submit/collect latency is amortized over K windows. **Default raised 64 → 128 (2026-09-17, +35% measured** on the quiet dev GPU, shift512 `p75_lex_m30`, 1 thread, fused chain, chunk 32, 45 s runs, order swapped: K=64 3902/3780 → K=128 5218/5175 win/s, identical 120.3 MR tests/window), parity-verified with the new default: 377 clean `windows=128 bad_windows=0 bad_pairs=0` flights, 0 bad). **Raised 128 → 256 (2026-09-18): +17% at shift512** (`p75_lex_m30`, 1 thread: 5264 → 6113 win/s, 119.7/122.6 → 141.4/143.3/141.5 mH/s) **and +19.4% at shift1017** (`p130_lex_m30`: 1498 → 1788 win/s), both parity-verified with the sliced replay). **VRAM is the binding constraint on 8 GB cards (measured 2026-09-19):** one production instance at shift509 with 2 workers and K=512 holds **4.9 GB**, so a second overlapping instance exceeds 8 GB and its `cudaMalloc` fails — the worker then **fails closed to the CPU sieve (~19 win/s instead of ~11.5k, i.e. ~600×)**. That failure now prints the needed size, the candidate capacity x K x limbs that set it, `cudaMemGetInfo` free/total, and the fix (`lower MINING_JUMP2_BATCH`) — and since 2026-09-21 that capacity is no longer the pathological `odd_interval x K` but the measured survivor density (see `GPU_EXTRACT_CAND_CAP`), so the same K costs ~4.4x less VRAM. Run one instance per 8 GB card, or lower K; K=256 halves the candidate buffers for ~10% less throughput. **Re-measured at the current chunk 12 (2026-09-19, shift507 `p74_lex_m30`, 100 s arms):** K=128 7,053 → K=256 9,773 → **K=512 11,192 win/s (+59%)** at an *unchanged* 79.6 MR tests/window — K grows the MR batch without adding a single MR test, which is exactly why it beats a bigger chunk. Batch fullness, not the round count, is the lever here: at K=128/chunk 12 a flight costs 34 rounds of which ~27 carry only ~12 candidates each (mean 297 per round) and the GPU retires 561k tests/s, versus 891k at K=512; a "widen the tail chunk" variant was derived from those round counts, implemented, and **refuted** (a round trip costs ~10 µs against a ~3-4 µs MR test, so the whole tail overhead is ~0.6% of wall). Worker count is dominated by batch too: **1** worker at K=1024 (8,434 win/s) beats **2** workers at K=128 (7,053). K is VRAM-bound on this 8 GB card at 512 with 2 workers (4.9 GB) — **K=1024 fails `cudaMalloc` at shift507 as well and falls closed to the CPU sieve**, so the compile-time cap was raised 512 → 1024 (2026-09-19) only to make K=1024 reachable on 12 GB cards, where 2 workers × K=1024 ≈ 6.4 GB fits (extrapolated ~12.9k win/s; not yet measured locally). **K=768 measured (2026-09-21, 2 workers, chunk 12, ABBA 70 s arms):** 11,601 / 11,630 win/s vs K=512 11,337 / 11,166 = **+3.2%** (overlapping ranges, so the sign is solid but not the size), at ~3.7 GB per worker — 2 workers = 7.4 GB, which fits an 8 GB card with a light desktop. This is why the cap raise matters on the dev box too: K=768 was simply unreachable before it. **After the buffer right-sizing (2026-09-21, same box/cover, ABBA 70 s arms):** K=512 11,374 / 11,606 → **K=2048 12,398 / 12,397 win/s (+7.9%, non-overlapping)**, K=4096 11,963 / 11,623 (the batch saturates there; one 9,553 reading was an outlier) — at an unchanged 79.5 MR tests/window, so the whole gain is `tests/s` (913k → 984k). The control arm that pins the OLD capacity (`GPU_EXTRACT_CAND_CAP=17916`) measured 11,261 win/s, i.e. the right-sized default is not a regression. Parity with the right-sized buffer: **211 flights, `windows=512 bad_windows=0 bad_pairs=0`, 0 buffer/OOM/fallback messages.** **Default raised 512 → 2048 (2026-09-21)** on the strength of that measurement (K=2048 +7.9%, K=4096 saturated), and the batch is now **clamped to the card at start-up**: each worker plans for its share `(free VRAM − 256 MiB) / ceil(nthreads / gpu_count)` and halves K (floor 256) until the candidate buffers fit, printing `[Worker 2] MINING_JUMP2_BATCH 4096 -> 2048 (candidate buffers need 3200 MiB at K=4096 but only 6418 MiB is free)`. Reason: an oversize K is not a correctness risk but a throughput one (cudaMalloc fails and the worker falls closed to the CPU sieve, ~600× slower), so the default must not be able to break on a smaller card or a higher shift. **Host arrays right-sized in the same pass (2026-09-22):** the per-flight offsets/offsets-sorted/is_prime arrays used to be `candidate_capacity × K` = one entry per ADDER per window (~36× more than the ~1,000 survivors a window actually has at shift507); they now use the same survivor-density rule as the device buffers and the sieve's capacity is pinned to the allocation (`gpu_sieve_set_cand_cap_limit`), so host and device cannot disagree and an extraction that does not fit fails closed instead of overrunning the host array. Measured effect at shift507/K=2048 with 2 workers: **peak RSS 674 MB** (the arrays alone used to be 2 × 1.32 GB), with no throughput change and parity still clean.
Verified: default K=2048 at shift507 with 2 workers (11.8k win/s) and with 3 workers (all three clamp to 2048, no OOM); at **4 workers the VRAM auto-clamp lands on K=1024** (4 x 1600 MiB at K=2048 does not fit an 8 GB card) and **K is a WASH there**, so the clamp's choice costs nothing: 4 workers at K=1024 is a CLEAN arm (all four workers at K=1024, no clamp) = 15,020 / 14,827 win/s, while a requested K=1536 could NOT be tested cleanly on this box — the auto-clamp is applied **per worker** from the free VRAM that worker sees at its own init, so the later workers dropped to K=768 / 384 and those mixed arms landed at 15,287 / 14,983, i.e. the same as K=1024 within the arm spread. Two consequences worth knowing: a larger K buys nothing measurable at 4 workers here, and **the startup log can show different `K=... windows` values for workers on the same card** (measured: 1536/1536/1536/384 and 512 x 5 / 256 x 3), so read each worker's own line instead of assuming they are equal. Parity at the new default is **54 flights, `windows=2048 bad_windows=0 bad_pairs=0`** (and 48 flights with the right-sized host arrays). Example: `MINING_JUMP2_BATCH=256 FUSED_GPU=1 ./bin/gapminer --crt-file data/crt/m23/shift509_p74_covermax_m38.txt --threads 2 --enable-gpu-fermat` |
| `MINING_JUMP2_CHUNK` | `12` | Survivor chunk size for one backward-search step (`8..512`). The chain tests a whole C-wide slice and keeps only **one** prime from it (the first when searching forward, the last when searching backward), so a bigger chunk means fewer rounds but more wasted MR tests per anchor, and a smaller chunk the reverse. **Default 12 since 2026-09-19 (was 32)** — measured on the dev host (RTX 3070, `shift509_p74_covermax_m38`, live difficulty ≈ 23.6, fused chain, 90 s arms, run-to-run spread ~4-5%): win/s at 2 threads = 10923 (8), **11161 / 11652 (12)**, 11052 (14), 10877 / 11457 (16), 11183 (20), 10636 (24), **9437 (32, the old default, old binary)**; at 4 threads 12 → **11680** vs 16 → 10742; at 1 thread 12 → 7298, 16 → 7168, 20 → **7774**, 32 → 7305. So the flat optimum is 12..20 (the ordering inside it is not resolvable at ±4-5%), 8 and ≥24 are clearly worse, and **chunk 12 is a safe single default: it matches the old default at ONE worker per GPU (7298 vs 7305) and is 18-21% faster at the production packing of 2+ workers per GPU**. Wasted MR tests per window scale with the chunk: 72.8 (8), 79.7 (12), 87.1 (16), 120.2 (32). **Chunk 12 re-confirmed at the now-default K=512 (2026-09-19, shift507 `p74_lex_m30`, 2 workers, 80 s arms):** 12 → **11,277** vs 24 → 10,616 win/s; the bigger chunk adds +29% MR tests per window (79.6 → 102.8) for a batch that K=512 already saturates, and pushes `extract` from 52.8% to 82.6% of wall. At large K keep the chunk at the small end. **Chunk 8 re-tested at large K (2026-09-21, ABBA 70-80 s arms, 2-3 workers):** it is a WASH once K is large — 1w/K=1024 8,003 (12) vs 7,731 (8) = −3.4%; 2w/K=512 11,252 vs 10,929 = −2.9%; 2w/K=768 11,616 vs 11,757 = +1.2%; 3w/K=512 13,068 vs 13,143 = +0.6% (arm spread ~±5%, so only the two negative arms resolve). Chunk 8 reliably cuts MR tests per window (79.5 → 72.5, −8.8%) but loses a comparable share of tests/s, because halving the chunk halves the per-round batch (K × C) and doubles the round count; the two effects cancel. Keep 12 — the earlier "8 is clearly worse" verdict was measured at K=128, where the batch was starved. Emitted-set parity vs a full scan is verified at chunks 12/16/24/32 (`bad_windows=0 bad_pairs=0`, ~900 flights each, `--merit 16`). Example: `MINING_JUMP2_CHUNK=16 FUSED_GPU=1 ./bin/gapminer --crt-file data/crt/m23/shift509_p74_covermax_m38.txt --threads 2 --enable-gpu-fermat` |
...
| `GPU_EXTRACT_CAND_CAP` | auto | Candidate-buffer capacity in **slots per window** (the extract/pack buffer that holds survivors, and whose size used to be one slot per odd position = the pathological "every odd slot is a survivor" bound). With the default (unset/0) the capacity starts at `max(4096, window/8)`, is right-sized after the FIRST measured window to `4 x survivors + 1024` (applied at the next flight boundary, never mid-flight), and GROWS (loudly, `slots/window` reported) if a later window needs more — it is never truncated, because a dropped survivor would make two non-consecutive primes look consecutive, i.e. a false gap. The kernel signals overflow in the high bit of the count word it already publishes, so the check costs no extra CUDA call. Measured survivor density at shift507 / 2M-3M sieve primes: **1,000.2 per 17,916-slot window** (380.2M survivors / 380.1k windows), so the old allocation was ~18x oversized; the effect is that the same `MINING_JUMP2_BATCH` now costs ~4.4x less VRAM (K=1024 went from "needs 2 x 1680 MiB, device free 1363 MiB → fails closed to the CPU sieve" to ~0.77 GB/worker), which is what lets K reach 2048-4096 at all. Setting a positive value pins the initial capacity (useful for experiments and for reproducing the old behaviour: `GPU_EXTRACT_CAND_CAP=17916` at shift507). The capacity is additionally pinned from BELOW by the callers' host arrays (`gpu_sieve_set_cand_cap_limit`, see `MINING_JUMP2_BATCH`/`GAP_HUNT_BATCH`): the host arrays hold the same dense layout, so the device may never be able to hold more than they do — an extraction that does not fit fails closed loudly instead of overrunning host memory. Correctness is not traded away: on a capacity that is reached the extraction fails that window closed (loudly) instead of dropping survivors. Parity with the right-sized buffer: 211 flights `bad_windows=0 bad_pairs=0`; `test_gpu_sieve` 32/32. Example: `GPU_EXTRACT_CAND_CAP=8192 FUSED_GPU=1 ./bin/gapminer --threads 2 --enable-gpu-fermat` |
...
| `GAP_HUNT_BATCH` | `1024` | GAP_HUNT windows per accumulated MR batch (`1..4096` since 2026-09-22, was `1..1024`; raised 32 → 64 on 2026-09-15, 64 → 128 on 2026-09-17, **64 → 512 on 2026-09-18**, 1024 → 4096 on 2026-09-22). **The K lever is now FLAT (measured 2026-09-22, dev 3070, shift998 `p128_covermax_m40`, min-merit 20, 45 s arms, no clamp fired): K=1024 1097 · K=2048 1089 · K=4096 1092 win/s** (last-6-tick spread ±2 %), i.e. the +32.5 % that 512 → 1024 bought does not continue — the walk is no longer batch-limited, so a bigger cap is not a lever (the cap was raised only to be able to measure the flattening; the default stays 1024). Consequence: raising it further needs no A/B. The two flight structs are `static` since then (at MAX=4096 each is ~12 MB and would overflow the 8 MB thread stack). Full sweep on the quiet dev GPU with the mode-aware bound below (jump2 on, min-merit 19, fixed k range 0..65535, emitted sets identical): shift507 **2452 (K=64) → 3156 (128) → 3601 (256) → 3824 win/s (512), +56%**; shift1017 **881 (128) → 1001 (256) → 1172 win/s (512), +33%**. Non-vacuous parity at min-merit 16 over k∈[0,131072): **15 gaps, set-identical** at K=512 vs K=64, 3434.6 vs 2291.8 win/s (**+49.9%**). A chain round pays a ~4 ms per-launch floor in the CGBN MR kernel, so more windows per round = cheaper rounds. **Device-memory cost is now `2 × cand_cap_per_window × K × limbs × 8 B`**, where `cand_cap_per_window` is the MEASURED survivor density (see `GPU_EXTRACT_CAND_CAP`: `max(4096, odd_interval/8)` at the first window, then 4× the measured count), printed by the walk banner as `vram_est=<MB>`. **Right-sized 2026-09-21 (the same change as the mining path):** the old formula reserved one slot per odd position, so shift1017 K=512 reported **6407 MB** and really used ≈7.3/8 GB; it now reports **967 MB** (an 8× honest correction) and K=1024 = **1934 MB**, which is why the old "on an 8 GB card at shift ≥ 1000 use `GAP_HUNT_BATCH=256`" advice no longer applies. Measured walk rate at shift998 `p128_covermax_m40`, min-merit 20, ABBA 40 s arms on the dev 3070: **K=128 773 win/s → K=512 971-1019 → K=1024 1025 win/s (+32.5% vs K=128)** — the K lever that the VRAM bound had blocked is now usable on an 8 GB card. Still fail-closed: an allocation failure is visible (`gpu_sieve: cands[0] alloc: out of memory`) and stops the walk, and a window denser than the reserved capacity GROWS the buffer loudly instead of dropping survivors. **Default raised 512 → 1024 (2026-09-21)** on the strength of that measurement, and the batch is now **clamped to the card at start-up** (same rule as the mining chain: halve K, floor 128, until the candidate buffers fit the free VRAM minus a 256 MiB reserve, with a loud `[GAP_HUNT] GAP_HUNT_BATCH X -> Y (...)` line). Verified at the new default: `batch=1024 vram_est=1934MB`, 1063 win/s, no clamp, no OOM. A walker cannot see other walkers on the same card (separate processes), so the clamp divides by the *process's* free memory only — when stacking several walkers on one GPU, lower `GAP_HUNT_BATCH` for all but the first. **Host arrays right-sized (2026-09-22):** the walk's offsets/flags arrays used to be `candidate_capacity × K`/per-adder (~16× oversized at shift998: `cumcap=48,380,928` at K=512), now the same survivor-density rule as the device buffers with the sieve pinned to it (`cumcap=6,046,720` at K=1024, i.e. 97 MB instead of 1.55 GB); a dense window fails closed rather than overrunning the host array. Measured: a K=1024 walker peaks at **456 MB RSS** and 617 emitted gaps at `--gap-hunt-min-merit 10` validate `bad=0` with `test_gap_hunt`. An allocation failure is visible (`gpu_sieve: cands[0] alloc: out of memory`) and stops the walk; it never falls back silently. The batch is bounded by the mode: with the full-scan head (`GAP_HUNT_JUMP`/`GAP_HUNT_JUMP2` off) it is clamped per window to the MR context's REAL per-call cap. Batch 128 measures **+19.6%** (shift507 `p74_lex_m30`, min-merit 19, fixed k range 0..65536, quiet GPU: 64 → 2435.1/2450.5 win/s, 128 → 2917.3/2924.8 win/s) and the emitted gap set is batch-independent at that setting, but **the default stays 64** because the larger setting only pays off when the batch fits the MR context's real cap: at min-merit 16 over k∈[0,131145) the K=128 and K=64 chains both emitted the same 15 gaps while the full scan emitted 11 — that discrepancy turned out to be **build hygiene, not gap logic** (a stale `gpu_adapter.o` carried the old 160000 candidate cap and every candidate past it was dropped silently; see `docs/BUILD_HYGIENE_INCIDENT.md`), and after the clean rebuild the same plain run gives 15/15 byte-identical to the chain. Batches are sized from the MR context's REAL per-call cap (`gpu_fermat_max_batch()`, printed as `mrcap=` in the walk banner), never from a compile-time macro: a stale object can carry an older cap and an oversize batch is then truncated silently (see `docs/BUILD_HYGIENE_INCIDENT.md`). The walk auto-reduces the batch when the density estimate does not fit and the fill fails closed |
