...
14,511 (3/card), **15,376 (4/card) = +16%**, and **+21.9%** against 2 workers at the
SAME K=1024 (12,613 -> 15,376); 8 workers on ONE card drops to 11,612 (-12%). Scope:
...
Verified: default K=2048 at shift507 with 2 workers (11.8k win/s) and with 3 workers (all three clamp to 2048, no OOM); at **4 workers the VRAM auto-clamp lands on K=1024** (4 x 1600 MiB at K=2048 does not fit an 8 GB card) and **K is a WASH there**, so the clamp's choice costs nothing: 4 workers at K=1024 = 15,020 / 14,932 win/s vs K=1536 = 15,287 / 14,983 (the larger K is +1.1%, inside the arm spread; ABBA, same protocol as the packing table under `--threads`). Parity at the new default is **54 flights, `windows=2048 bad_windows=0 bad_pairs=0`** (and 48 flights with the right-sized host arrays). Example: `MINING_JUMP2_BATCH=256 FUSED_GPU=1 ./bin/gapminer --crt-file data/crt/m23/shift509_p74_covermax_m38.txt --threads 2 --enable-gpu-fermat` |
