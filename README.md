...
(tunable via `GAP_HUNT_BATCH`, 1..128) with two alternating flights: the host
...
| `MINING_JUMP2_BATCH` | `128` | Windows per chain flight (`1..128`; the compile-time cap must stay 128 — raising `MINING_JUMP2_BATCH_MAX` to 256 was tried on 2026-09-17 and IMMEDIATELY REVERTED because the chain's emitted set then stops matching the full scan (`MINING_JUMP2_VERIFY=1` at K=256: `bad_windows=75..83 bad_pairs=14..28` after clean first flights), i.e. something downstream is still 128-wide and degrades silently; the apparent +15% at K=256 measured a wrong path). The per-round gather/submit/collect latency is amortized over K windows. **Default raised 64 → 128 (2026-09-17, +35% measured** on the quiet dev GPU, shift512 `p75_lex_m30`, 1 thread, fused chain, chunk 32, 45 s runs, order swapped: K=64 3902/3780 → K=128 5218/5175 win/s, identical 120.3 MR tests/window), parity-verified with the new default: 377 clean `windows=128 bad_windows=0 bad_pairs=0` flights, 0 bad) |
