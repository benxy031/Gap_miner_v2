...
| `FUSED_GPU` | off | Full GPU-resident CRT pipeline (sieve+extract+MR on-device; implies `GPU_SIEVE`, defaults to a 5M deep sieve) |
...
| `GAPDEBUG` | off | CRT HALF_CLASS gap diagnostics: for every emitted gap, log to stderr the window class, gap class endpoints, and the interior candidates with their MR flags (used to trace false-gap regressions; verbose — development only) |
