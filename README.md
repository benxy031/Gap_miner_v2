...
| `GPU_FERMAT_CGBN_SOA` | `1` | Layout of the CGBN MR candidate buffer: `1` = structure-of-arrays (`cgbn_fermat_kernel_soa_t`), `0`/`n`/`f` = array-of-structures. **Measured 2026-09-24** (devbox 3070, `bench_fermat 12 <batch> 12`): AoS is **+11.3% at batch 2000 (1,003k vs 901k cand/s) and +7.5% at batch 20000 (1,453k vs 1,352k)** — but in the production pipeline (`FUSED_GPU=1`, `shift507_p74_lex_m30`, 3 workers, 60 s arms) the difference is **0.03% (14,259 vs 14,255 win/s, GPU acc/wall 0.915 vs 0.913)**, because that configuration is GPU-*and*-host balanced and absorbs a pure kernel-layout win. The default therefore stays SoA (the fused path's extraction writes candidates in that order). Example: `GPU_FERMAT_CGBN_SOA=0 ./bin/bench_fermat 12 20000 12` |
