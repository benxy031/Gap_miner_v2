...
|------:|-------:|--------------:|----------------:|---------:|-----------:|-----------:|-----------:|
|    26 |      8 |            19 |            23.2 |        2 |       4105 |       4301 |       4887 |
|    37 |     10 |            29 |            32.6 |        4 |       4265 |       4469 |       5078 |
...
|   128 |     26 |           101 |           127.5 |        0 |       5590 |       5856 |       6655 |
|   133 |     26 |           101 |           127.5 |        5 |       5663 |       5932 |       6741 |
|   160 |     30 |           113 |           154.5 |        5 |       6056 |       6344 |       7209 |
...
|   520 |     76 |           383 |           517.6 |        2 |      11296 |      11834 |      13448 |
|   640 |     89 |           461 |           631.2 |        8 |      13043 |      13664 |      15527 |
|   720 |     98 |           521 |           711.6 |        8 |      14207 |      14884 |      16913 |
|   768 |    104 |           569 |           766.2 |        1 |      14906 |      15616 |      17745 |
|   896 |    118 |           647 |           895.8 |        0 |      16769 |      17568 |      19963 |
...
| `--fitness-candidate-guard G` | Candidate-count guard-band, used by **both** `probability` and `max-run` modes (default: `2`). Larger values prefer fewer candidates more strongly. In `max-run` mode this stops the search from accepting a solution with far more candidates for only a marginally larger `max_spacing` (see [Optional Max-Run Fitness Mode](#optional-max-run-fitness-mode) for why this guard was added). |
...
**Correctness note (fixed 2026-08-13): unbounded candidate-count drift.**
`--fitness-candidate-guard` bounds *each individual acceptance* to at most
`guard` more candidates than the current champion. Every accumulation loop
in `gen_crt` (greedy-restart pool per thread, the pool merge across
threads, evolutionary population insertion, and ILS round acceptance) used
that *current champion* as the guard anchor — but the champion itself can
already have drifted upward from an earlier acceptance. Across many
restarts/generations/rounds (hundreds to tens of thousands), this let the
selected candidate count random-walk upward with no overall bound, even
though every single step individually stayed within the guard. A true
best-ever-seen counter was already tracked in most of these loops for
progress display, but was never used to cap acceptance. Measured effect at
shift 64 / merit 45 / 15 primes / `--ctr-strength 2000 --ctr-ivs 200`:
`probability` mode systematically converged to ~1336-1340 candidates
across repeated runs versus `candidate` mode's ~1333-1335 at the same
budget. Fix: every acceptance is now additionally gated on
`n_candidates <= best_ever_min_candidates + candidate_guard`, where
`best_ever_min_candidates` only ever decreases. After the fix,
`probability` mode converges to the same ~1332-1333 range as `candidate`
mode. This also applies to `max-run` mode (see below), which uses the same
guard mechanism.

**Performance note:** the single/pair/triple local-search steps used by
`probability` mode compute their `n_candidates` and kernel score via
incremental deltas against a precomputed "all other primes" baseline
(same technique the default `candidate` mode already used), rather than
calling the full evaluator once per candidate offset. Earlier versions
called the full evaluator per candidate inside these loops, which for
large prime counts (e.g. shift 1001 / 129 primes) made pair/triple local
search effectively never finish (an O(p), O(pa\*pb), and O(pa\*pb\*pc)
blowup respectively — a real-world run at that scale did not complete in
8+ hours). After the fix the same shift 1001 / 129 primes / `--ctr-fixed
15` case completes in bounded, predictable time (measured ~37 min at
`--ctr-strength 200 --ctr-ivs 50` vs. ~2.5 min for `candidate` mode at the
same settings — roughly 15x slower, not orders of magnitude / unbounded).
The residual overhead comes from evaluating both the narrow and the wide
window every local-search step; for very large prime counts, consider
reducing `--ctr-strength` / `--ctr-ivs` relative to what you'd use for
`candidate` mode.
...
- Collects all uncovered positions in `[1, gap_target]` and finds the largest
  ...
  endpoints are candidates that could be prime and everything strictly
  between them is definite composite **with respect to the CRT primes
  only** — `max_spacing` is an upper bound on the largest prime gap this
  offset set could ever produce, not a prediction of a typical or likely
  gap. It says nothing about compositeness from primes larger than the
  largest CRT prime, which is what actually determines whether real,
  much-larger gaps show up during mining.
- The optimizer maximises `max_spacing` (internally stored as
  `opt_score = -max_spacing`, so minimising `opt_score` maximises spacing),
  guarded by `--fitness-candidate-guard` (default `2`): a larger
  `max_spacing` is only accepted if it doesn't cost more than `guard` extra
  `n_candidates` versus the alternative; otherwise the fewer-candidates
  solution wins regardless of `max_spacing`. Final tie-break is fewer
  `n_candidates`.
- **Known limitation (fixed 2026-08-13):** before the guard band was added,
  this mode had no tolerance at all and would trade away many extra
  candidates (i.e. materially more Fermat-test overhead every window) for a
  spacing gain as small as 1 unit. A controlled A/B at shift 64 / merit 45 /
  15 primes (`--ctr-strength 2000 --ctr-ivs 200`, same budget both runs)
  measured `probability` mode converging to `1333` candidates and unguarded
  `max-run` mode converging to `1348` (+15, i.e. +1.1% more Fermat tests per
  window) for a `max_spacing` of only `60` out of a `gap_target` of `9982`
  — nowhere near large enough to matter, since real observed gaps in mining
  logs regularly exceed `4000`+ (driven by larger untested primes, not by
  `max_spacing`). Adding the guard band alone was not sufficient, though:
  the guard also needed the anchor fix described in the Probability Fitness
  Mode section above (it was initially anchored to the current champion,
  which can itself drift). With both fixes, the same run now converges to
  the `1332`-`1336` range (on par with `probability`/`candidate` mode) with
  `max_spacing` in the `30`-`44` range. In short: for typical CRT mining,
  prefer `candidate` or `probability` mode; `max-run` is only useful when a
  single record-length gap independently matters more than overall
  Fermat-test throughput, and even then the guard band keeps its
  Fermat-test cost close to the candidate-minimising baseline.
- Unlike `candidate` and `probability` mode, this does **not** primarily
  ...
  density. With the default guard band, expect `n_candidates` close to the
  `candidate`-mode best rather than materially higher, for the same
  shift/merit.
...
```bash
./bin/gen_crt --calc-ctr \
  --ctr-primes 76 --ctr-merit 22 --ctr-bits 2 \
  --ctr-strength 10000 --ctr-evolution --ctr-fixed 13 --ctr-ivs 1000 \
  --ctr-file crt/crt_s520_m22.txt
```

Expected: ~787 candidates.
