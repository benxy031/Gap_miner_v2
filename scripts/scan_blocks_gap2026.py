...
def print_block(blk, csv=False, verbose=False, rec_db=None):
    ...
    if rec_db is not None and gaplen > 0:
        if gaplen in rec_db:
            ...
            # Novi rekord samo ako je merit STROGO veći na 4 decimale
            # (preciznost merits.txt).  delta == +0.0000 znači isti gap/merit
            # kao postojeći rekord → NIJE rekord.
            is_record = round(merit, 4) > round(rec_merit, 4)
        else:
...
def _write_export(path, rows):
    """
    Zapiši rekorde u FILE za ručno slanje na primegaps.cloudygo.com.

    Format (jedan red po rekordu):
        <gap> <merit> <start_prime>

    PAŽNJA: sajt ne prihvata sirove decimalne proste brojeve u
    <start_prime> — jednolinijski POST se tiho ignoriše, a višelinijski
    batch vraća HTTP 500.  Pre lepljenja svaki broj treba konvertovati u
    izraz formata a * P# / d - b (npr. 3676117443599 * 337# / 2310 - 7578).
...
def main():
    ...
    for blk in iter_blocks(conn, start, end):
        ...
        # Ista tolerancija kao u print_block: samo strogo bolji merit na
        # 4 decimale je rekord; isti gap/merit kao postojeći rekord nije.
        is_rec = (rec_merit_val is not None and
                  round(merit, 4) > round(rec_merit_val, 4))
        if (args.records_only or args.export_records or args.submit_records
                or args.submit_dry_run) and rec_db is not None:
            if not is_rec:
                continue
        ...
        if (args.export_records or args.submit_records
                or args.submit_dry_run) and is_rec:
            gs = blk.get("gapstart", "")
            if gs:
